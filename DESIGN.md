  ...
  option-chip:
    ...
    announcementPolicy:
      ...
      proofEvidence:
        ...
        method: "local Playwright headless preview"
        passed: "native button elements; Chromium Accessibility tree maps the controls to role button with pressed/disabled state; Space activation; selected state/status update; focus remains on activated chip; disabled unavailable state does not activate; status region present; no horizontal overflow at 220px/280px/320px; no console errors"
        notProven: "actual screen-reader output, browser/assistive-technology interoperability, application runtime, and implementation acceptance"
        styleguideProof:
          status: "UNVERIFIED"
          checkedAt: "2026-09-21"
          method: "Playwright browser smoke on /auis/styleguide at desktop and 390px"
          passed: "OptionChip renders in the scoped styleguide showcase; ABS starts selected; Resin is disabled with an explicit unavailable label; Space selects PLA and focus remains on PLA; material price status updates; option-chip proof has no horizontal overflow at 390px; narrow reflow frame is visible"
          notProven: "actual screen-reader speech/output, browser/assistive-technology interoperability, and product runtime acceptance"
      scope: "announcement policy is final: native button state is authoritative; use one concise adjacent role=status region only for material downstream changes; runtime/assistive-technology acceptance and product propagation are separate evidence gates"
    runtimeAcceptanceGate:
      ...
      scope: "acceptance criteria are final; current runtime/assistive-technology evidence is UNVERIFIED and implementation/product propagation remain separate gates"
      currentEvidence:
        status: "UNVERIFIED"
        checkedAt: "2026-09-21"
        finding: "A dedicated styleguide-only option-chip implementation is now registered and rendered; actual screen-reader/browser acceptance and runtime acceptance remain unverified"
...
metadata:
  ...
  decisionClosure:
    ...
    optionChip:
      ...
      evidence: "visual acceptance APPROVED; local structural and runtime/AT evidence UNVERIFIED"
    cardAndPatterns:
  ...
  evidenceBoundary:
    ...
    optionChip:
      ...
      localStructural: "PARTIAL; native button, pressed/disabled state, keyboard interaction, focus retention, and narrow reflow are evidenced"
      runtimeAssistiveTechnology: "UNVERIFIED; actual screen-reader speech/output and browser/assistive-technology interoperability are not claimed"
...
| Controls | Labelled 44px/12px/16px/8px; input 44px/10px/12px/8px; icon-only 44×44px/20px icon; badge noninteractive; option chip Candidate B | Touch, screen-reader, zoom, and application runtime evidence `UNVERIFIED` |
...
DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE `APPROVED` untuk visual acceptance pada proof styleguide yang dinamai; runtime/AT dan propagasi product screen tetap `UNVERIFIED`. Outline/ghost memiliki approval
...
DS-DEC-008-OPTION-CHIP-IMPLEMENTATION `APPROVED` untuk implementation proof styleguide-only; product-screen propagation dan official promotion tetap `UNVERIFIED`. Outline/ghost memiliki approval
DS-DEC-008-OPTION-CHIP-AT-PROOF `UNVERIFIED` untuk evidence local keyboard/DOM/overflow; DS-DEC-008-OPTION-CHIP-RUNTIME-AT-GATE `APPROVED` sebagai kriteria acceptance; actual screen-reader output dan runtime acceptance adalah evidence `UNVERIFIED`. Outline/ghost memiliki approval
...
| DS-DEC-008 | Cards / badges / patterns / `UNVERIFIED` | Card interaction boundary ditutup pada DS-DEC-008-CARD-INTERACTION; option-chip role/behavior ditutup pada DS-DEC-008-OPTION-CHIP; Candidate B visual direction, anatomy, reflow policy, announcement policy, runtime/assistive-technology acceptance gate, dan styleguide-only implementation permission ditutup pada DS-DEC-008-OPTION-CHIP-VISUAL, DS-DEC-008-OPTION-CHIP-ANATOMY-GATE, DS-DEC-008-OPTION-CHIP-REFLOW-GATE, DS-DEC-008-OPTION-CHIP-ANNOUNCEMENT, DS-DEC-008-OPTION-CHIP-RUNTIME-AT-GATE, dan DS-DEC-008-OPTION-CHIP-IMPLEMENTATION; local structural proof dicatat `UNVERIFIED` pada DS-DEC-008-OPTION-CHIP-AT-PROOF; badge wrapping ditutup pada DS-DEC-008-BADGE-REFLOW; remaining pattern relevance menjadi evidence UNVERIFIED | Contract card/pattern sesuai kebutuhan Niuva, termasuk token mapping detail, marker gap, thumbnail/price anatomy, exact threshold/collision proof, announcement copy/mechanism, actual screen-reader/runtime acceptance, dan elemen yang benar-benar diperlukan |
...
| DS-DEC-008-OPTION-CHIP | Option chip role and behavior / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: option chip terpisah dari badge, memakai native button semantics, target minimum 44px, serta state selected/unavailable yang terlihat | Role/behavior policy ditutup; Candidate B direction ditutup pada DS-DEC-008-OPTION-CHIP-VISUAL; styleguide-only implementation permission ditutup pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION; exact anatomy, dimensions, thumbnail/price anatomy, group reflow, state announcement, keyboard/assistive-technology acceptance, runtime, dan product propagation tetap `UNVERIFIED` |
| DS-DEC-008-OPTION-CHIP-VISUAL-GATE | Option chip visual proof gate / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: selected/unavailable wajib memiliki cue persisten yang tidak bergantung pada warna; option chip tetap terpisah dari badge; gate dibuktikan melalui keyboard focus, grayscale/contrast, representative content, dan narrow-screen reflow | Gate ditutup; Candidate B direction dipilih pada DS-DEC-008-OPTION-CHIP-VISUAL; exact token values, marker gap, thumbnail/price anatomy, state announcement, assistive-technology proof, dan runtime tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION |
| DS-DEC-008-OPTION-CHIP-INTERACTION-GATE | Option chip keyboard/state communication proof gate / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: native button keyboard behavior (Tab/Enter/Space), focus tetap setelah selection, unavailable tidak dapat diaktifkan, dan selected/unavailable tersedia melalui semantic state/status | Gate ditutup; exact state mapping, announcement copy/mechanism, assistive-technology proof, dan runtime tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION |
| DS-DEC-008-OPTION-CHIP-VISUAL | Option chip visual direction / `APPROVED`; option-chip contract `COMPLETE` | Owner memilih Candidate B pada 2026-09-21 setelah preview A/B: surface putih, quiet outline, selected marker + underline cue, dan explicit unavailable label | Visual direction ditutup; anatomy/dimensions ditutup pada DS-DEC-008-OPTION-CHIP-ANATOMY-GATE; token mapping, label copy, announcement, dan runtime proof tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION |
| DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE | Styleguide visual acceptance / `APPROVED`; option-chip contract `COMPLETE` | Owner menerima preview aktual `/auis/styleguide#option-chip-proof` pada 2026-09-22 untuk state selected, unavailable, dan narrow-screen reflow | Visual acceptance ditutup untuk proof styleguide yang dinamai; screen-reader/runtime acceptance, product propagation, dan official promotion tetap `UNVERIFIED` |
| DS-DEC-008-OPTION-CHIP-ANATOMY-GATE | Option chip anatomy and geometry / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: radius 8px, minimum height 44px, padding 8px/12px, marker 18px, label wrap pada batas kata tanpa truncation/shrink, lebar content-driven dalam container, dan source order dipertahankan saat reflow | Anatomy/geometry gate ditutup; marker gap, token mapping detail, label copy, thumbnail/price anatomy, group reflow proof, announcement, dan runtime tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION |
| DS-DEC-008-OPTION-CHIP-REFLOW-GATE | Option chip group reflow policy / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: group wrap dalam source order, tanpa horizontal scroll/truncation/font shrinking; unavailable label boleh wrap tetapi tetap eksplisit; proof memakai label nyata pada frame 220px/280px/320px | Reflow policy ditutup; exact group gap, container threshold, collision proof, focus order runtime, dan assistive-technology proof tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION |
| DS-DEC-008-OPTION-CHIP-ANNOUNCEMENT | Option chip announcement policy / `APPROVED`; option-chip contract `UNVERIFIED` | Owner menyetujui pada 2026-09-21: semantic state pada control yang sama; label + state singkat tanpa duplicate live announcement; unavailable tidak activate; adjacent status hanya untuk downstream change yang material; focus tidak berpindah | Policy ditutup; exact native/ARIA mapping, copy, announcement mechanism, assistive-technology proof, dan runtime tetap `UNVERIFIED`; implementation permission dicatat pada DS-DEC-008-OPTION-CHIP-IMPLEMENTATION; local structural proof dicatat pada DS-DEC-008-OPTION-CHIP-AT-PROOF |
| DS-DEC-008-OPTION-CHIP-AT-PROOF | Local option chip interaction/accessibility evidence / `UNVERIFIED` | Pada 2026-09-21 local preview dan browser smoke `/auis/styleguide` lulus untuk native button, Space activation, state/status update, focus retention, disabled unavailable, explicit label, narrow reflow/no horizontal overflow, dan zero console errors; Chromium Accessibility Tree memetakan control sebagai role `button` dengan state `pressed`/`disabled` yang sesuai | Evidence structural ditutup sebagian; actual screen-reader output, browser/assistive-technology interoperability, application runtime acceptance tetap `UNVERIFIED`; visual acceptance proof disetujui Owner pada 2026-09-22 melalui DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE |
| DS-DEC-008-OPTION-CHIP-RUNTIME-AT-GATE | Option chip runtime/assistive-technology acceptance gate / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21: acceptance harus memakai actual screen-reader/browser pair dalam runtime yang disetujui atau scoped acceptance preview; mencakup keyboard entry/activation, selected/unavailable, focus retention, concise announcement, material downstream status, dan narrow-screen reflow | Gate kriterianya ditutup; styleguide-only implementation tercatat melalui DS-DEC-008-OPTION-CHIP-IMPLEMENTATION; runtime environment, exact native/ARIA mapping, test tooling, actual screen-reader output, dan acceptance result tetap `UNVERIFIED` |
| DS-DEC-008-OPTION-CHIP-IMPLEMENTATION | Option chip styleguide implementation authorization / `APPROVED`; option-chip contract `COMPLETE` | Owner menyetujui pada 2026-09-21 implementation proof scoped ke `/auis/styleguide#option-chip-proof`, memakai native button, Candidate B, minimum target 44px, selected/unavailable semantics, dan source-order reflow; product-screen propagation tidak termasuk | Implementation permission ditutup untuk styleguide-only; implementation acceptance, actual screen-reader/runtime proof, exact token/copy mapping, product propagation, dan official promotion tetap `UNVERIFIED`; visual acceptance proof ditutup Owner pada 2026-09-22 melalui DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE |
...
| `configurator-option-chip` | Badge/chip concept | Ada sebagai mapping | COMPLETE | APPLICABLE | Role/behavior, native button semantics, dan minimum target 44px disetujui melalui DS-DEC-008-OPTION-CHIP; Candidate B visual direction dan anatomy (8px radius, 44px minimum, padding 8px/12px, source-order wrap) serta no-horizontal-scroll reflow policy disetujui; styleguide-only implementation proof dan visual acceptance mengikuti DS-DEC-008-OPTION-CHIP-IMPLEMENTATION serta DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE; runtime/assistive-technology acceptance gate mengikuti DS-DEC-008-OPTION-CHIP-RUNTIME-AT-GATE; token mapping and geometry are final; collision, screen-reader, and runtime evidence are UNVERIFIED |
| `configurator-option-chip-selected` | Selected chip state | Ada sebagai mapping | COMPLETE | APPLICABLE | Candidate B disetujui: quiet outline, marker/underline cue, dan persistent non-color distinction; 18px marker, label wrapping, source order, dan unavailable-label behavior mengikuti anatomy/reflow gate; native keyboard, focus retention, semantic state/status, dan concise non-duplicative announcement mengikuti interaction/announcement gate; styleguide-only implementation proof dan visual acceptance mengikuti DS-DEC-008-OPTION-CHIP-IMPLEMENTATION serta DS-DEC-008-OPTION-CHIP-VISUAL-ACCEPTANCE; runtime/assistive-technology acceptance gate mengikuti DS-DEC-008-OPTION-CHIP-RUNTIME-AT-GATE; local structural proof `PARTIAL` pada DS-DEC-008-OPTION-CHIP-AT-PROOF; exact mapping and copy are final; screen-reader and runtime evidence are UNVERIFIED |
...
- OptionChip memiliki visual acceptance `APPROVED` hanya pada
  `/auis/styleguide#option-chip-proof`; evidence structural/keyboard saat ini
  tetap `PARTIAL`, sedangkan screen-reader/runtime acceptance dan product
  propagation tetap `historical unresolved`.
